...
export async function generateThumbnail(data, script, options = {}) {
  ...
  const imageBlob = await client.textToImage({
    ...
    model,
    inputs: buildThumbnailPrompt(data, script),
...
export async function handleGenerateThumbnail(request, response) {
  try {
    ...
    await updateScriptThumbnail(existingScript.id, auth.user.id, thumbnail);
